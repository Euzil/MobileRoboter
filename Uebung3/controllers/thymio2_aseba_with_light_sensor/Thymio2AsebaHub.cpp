...
Thymio2AsebaHub::Thymio2AsebaHub(int port, int nodeID) : stream(0) {
  thymio2 = this;

  vm.nodeId = nodeID;
  cout << "NodeID: " << vm.nodeId << "." << std::endl;
...
void Thymio2AsebaHub::incomingData(Dashel::Stream *stream) {
  ...
  if (bswap16(*(uint16 *)&lastMessageData[0]) >= 0xA000)
    ...
    //I wonder if commenting this out, instead of dealing with the error, will backfire on me later...
    return;
}
